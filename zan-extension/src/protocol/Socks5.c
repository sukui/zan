...
int swSocks5_connect(swClient *cli, char *recv_data, int length)
{
    ...
    char *buf = ctx->buf;
    swWarn("connect %d",ctx->state);
    uchar version = recv_data[0];
    uchar method = recv_data[1];
    swWarn("test2 %X %X",version,method);
    if (ctx->state == SW_SOCKS5_STATE_HANDSHAKE)
    {
        ...
        {
            swError("SOCKS version is not supported.");
            return SW_ERR;
        }
    ...
    else if (ctx->state == SW_SOCKS5_STATE_AUTH)
    {
        ...
        if (status != 0)
        {
            swError("SOCKS username/password authentication failed.");
            return SW_ERR;
        }
...
//        uint16_t port = *(uint16_t *) (recv_data + 8);
        ...
        {
            swError("Socks5 server error, reason :%s.", swSocks5_strerror(result));
        }
        return result;
    }
    return SW_OK;
